...
* First time you start the tool if HdfmMonkey is not present, the log Windows will throw an error message reporting it is missing (as expected). Either download &  install manual hdf monkey or use the Download and install button. If py hdfm gooey fails to download HdfmMonkey and your internet connectivity works fine on Windows it is likely because the Windows firewall (or in your own antivirus firewall settings) prevents outbound call from you Python executable. To solve this you may provide temporary access to the python dot exe file to the internet using the Windows Defender firewall control panel built in.
